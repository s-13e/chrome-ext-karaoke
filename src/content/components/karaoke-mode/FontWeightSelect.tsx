...
import { AVAILABLE_FONT_WEIGHTS, getFontWeightLabel, DEFAULT_FONT_WEIGHT } from '@constants/fontWeights';
...
interface FontWeightSelectProps {
  ...
  className?: string;
  /** 값이 설정되지 않았을 때 표시할 기본 font-weight (기본값: DEFAULT_FONT_WEIGHT) */
  defaultValue?: number;
}

export const FontWeightSelect: React.FC<FontWeightSelectProps> = memo(
  ({ value, onChange, className, defaultValue = DEFAULT_FONT_WEIGHT }) => {
    const selectRef = useRef<HTMLSelectElement>(null);
    const { t } = useTranslation();

    // value가 숫자인 경우 label로 변환, undefined면 defaultValue 사용
    const currentLabel = value !== undefined ? getFontWeightLabel(value) : getFontWeightLabel(defaultValue);
